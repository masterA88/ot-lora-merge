...
Interference-Resolved Merging of Low-Rank Adapters}\\[4pt]
\large\emph{Extended Edition with a Complete Step-by-Step Replication Record}}

\author{
Hilmi\\
\url{https://master-hilmi.vercel.app/}
}
\date{June 2026}
...
all configurations, and every run log. A complete step-by-step replication record, the probe-first
methodology, every probe, the full \texttt{torch}/CUDA environment-debugging sequence, every failed kernel
version and its fix, and the exact free-compute recipe, is given in Appendix~\ref{app:journey}, so that a
reader with only a free Kaggle account can reproduce the entire study.
...
\midrule
OT \emph{averaging} does not beat the naive floor & $0.61$ to $0.63$ at all ranks/scales & \ref{sec:results-averaging} \\
...
Compute-frugal by construction & $264$ to $359$\,s CPU, full model & \ref{sec:results-compute} \\
\bottomrule
\end{tabular}
\end{table}
...
projections, $m{=}n{=}768$, $r{=}16$, $k{=}128$) a full-model merge takes $\approx 4$ to $6$ minutes on a
...
predicts a few minutes single-core, matching the measured $264$ to $359$\,s in
...
task arithmetic, and tuned TIES, cluster at $\approx 0.63$ to $0.64$ normalized accuracy. This is the
...
\midrule
\emph{(reference)} simple average & n/a & 0.636 \\
\bottomrule
\end{tabular}
\end{table}
...
accuracy surface flat ($0.70$ to $0.708$) around it.
...
fallback when no shared ambient frame is assumed. Both merges are pure CPU, $264$ to $359$ seconds for the
full model.
...
Across all merges, the full-model merge cost is $\approx 4$ to $6$ minutes on a single CPU core
...
controlled $+0.7$ to $1.9$ point OT-over-SVD advantage is what our experiments actually license us to
...
The merge-time column of Table~\ref{tab:hetero} ($264$ to $359$\,s, single CPU core, full model) should be
...
most direct route to test whether the controlled $+0.7$ to $1.9$ point advantage carries through to beating
...
% =====================================================================
\section{The complete experimental journey: a step-by-step replication record}
\label{app:journey}
This appendix is a faithful, chronological lab notebook of how the entire study was carried out on free
compute, including every probe, every failed run, and every fix. We include it because (i) the
environment pitfalls below are non-obvious and will block any naive replication, and (ii) the
\emph{probe-first, fail-fast} methodology is itself transferable. A reader who follows this section can
reproduce the project end-to-end on a free Kaggle account. Throughout, ``kernel'' means a Kaggle
notebook/script run; merges are CPU; the GPU is used only to evaluate merged models.

\subsection{Methodology: probe-first, fail-fast, free-compute}
\label{app:method-meta}
The guiding principle was to never spend GPU quota on an uncertainty that a cheap (no-GPU) probe could
resolve first. Concretely: (1) build and unit-test the entire CPU merge core \emph{offline} (no GPU, no
network); (2) resolve every external-API unknown (adapter repository IDs, PEFT key layout, evaluation
harness construction, algorithm constructor signatures) with \emph{GPU-off} probe kernels; (3) only then
launch a GPU kernel, and even then write incremental logs and wrap every evaluation in
\texttt{try/except} so a single failure never discards a whole run's partial results. This discipline is
why, despite a long environment-debugging saga (Appendix~\ref{app:m0saga}), the total GPU time fit inside
one week of the free Kaggle quota.

\subsection{Phase 0, framing and the gap}
The project began as a literature-scan to find a free-compute, referee-credible contribution at the
intersection of LoRA fine-tuning and optimal transport. The scan established that OT is nearly absent
from the merging literature (one explicit OT-merging paper, OTMF \citep{pan2025otmf}, transports
activations not weights; the classic \citet{singh2020fusion} aligns full-layer neurons), and that the
crowded LoRA-merging cluster resolves interference by SVD/orthogonality, never by transport in the
rank-$r$ subspace. This produced the thesis of the paper and the decision to target the standard
$8$-task ViT-B/32 LoRA benchmark on which KnOTS/Core Space/GeoMerge report.

\subsection{Phase 1, the CPU optimal-transport core, built and tested offline}
Before touching any GPU we implemented the entire merge core as a pure-NumPy package
(\texttt{ot\_lora\_merge}: \texttt{directions, cost, ot\_solve, align, barycenter, merge, ties\_merge,
metrics, peft\_io}) and validated it with $17$ unit tests offline, including the identity guarantee
(Lemma~\ref{lem:identity}), Sinkhorn marginal constraints, the $\varepsilon\!\to\!0\approx$ EMD limit,
the rank-$r$ thin-SVD reconstruction (Lemma~\ref{lem:thinsvd}), and the GW heterogeneous-rank path. The
core is import-light (it does not pull \texttt{torch}); only the evaluation boundary (\texttt{peft\_io},
the FusionBench hook) needs \texttt{torch}. We also profiled the barycenter, finding and fixing a
slow path (a per-iteration dense $768\times768$ SVD) by switching to the factored thin-SVD of
Lemma~\ref{lem:thinsvd}, taking the worst-case layer barycenter from $\sim\!19$\,s to $\sim\!3.5$\,s.

\subsection{Phase 2, five no-GPU probes that de-risked the GPU runs}
\label{app:probes}
We ran five GPU-off probe kernels, each isolating one external-API uncertainty
(Table~\ref{tab:probes}). The decisive findings were: (probe~1) the FusionBench-bundled LoRA pool is
ViT-B/\textbf{16}, not B/32, and the assumed \texttt{tanganke/clip-vit-base-patch32\_*\_lora} repositories
\emph{do not exist}; (probe~3) the B/32 adapters that define the published bar are released separately by
the KnOTS authors at \texttt{hoffman-lab/KnOTS-ViT-B-32\_lora\_R16\_<task>}, with $r{=}16$, $\alpha{=}16$
(scale $1.0$), targeting $\{q,k,v,\text{out}\}_{\text{proj}}$, and a null \texttt{base\_model\_name}
field; (probe~4) FusionBench's \texttt{load\_lora\_vision\_model\_hf} loads these adapters natively and
our \texttt{peft\_io} parser extracts $48$ factor matrices per adapter with the expected shapes;
(probe~5) the FusionBench task pool cannot be built by Hydra \texttt{compose} (it is a config \emph{group},
not a primary config), so it must be instantiated directly from its \texttt{\_template} structure, and the
method constructors require explicit arguments (\texttt{TaskArithmeticAlgorithm(scaling\_factor)},
\texttt{TiesMergingAlgorithm(scaling\_factor, threshold, remove\_keys, merge\_func)}).

\begin{table}[h]
\centering
\caption{The five no-GPU probes and what each resolved before any GPU spend.}
\label{tab:probes}
\small
\begin{tabular}{p{0.6cm} p{5.2cm} p{8.0cm}}
\toprule
\# & Purpose & Decisive finding \\
\midrule
1 & FusionBench API names + bundled LoRA config & bundled pool is B/16; \texttt{tanganke} B/32 LoRA repos
do not exist; all wired API names import \\
2 & PEFT key layout of a B/16 adapter & keys \texttt{base\_model.model.\dots.lora\_\{A,B\}.default.weight};
$A{:}(r,d)$, $B{:}(d,r)$; scale $\alpha/r$ \\
3 & Locate the B/32 ``bar'' adapters & KnOTS collection \texttt{hoffman-lab/KnOTS-ViT-B-32\_lora\_R16\_*};
$r{=}16$, $\alpha{=}16$, target $q,k,v,\text{out}$; weights as \texttt{.bin} \\
4 & Does FusionBench load KnOTS + does our parser read it & yes, natively; $48$ factors/adapter, shapes
confirmed; no shim needed \\
5 & FusionBench Hydra taskpool + method API & taskpool from \texttt{\_template} (not \texttt{compose});
exact method constructor signatures and default args \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Phase 3, the backbone decision (and why it matters)}
Probes 1 and 3 forced a decision: the zero-training FusionBench pool is B/16, but the \emph{published
bar} (GeoMerge $0.771$, Core Space $0.764$, KnOTS $0.740$) is measured on B/32 KnOTS-lineage adapters.
Using B/16 would forfeit direct comparability to that bar. We therefore chose the B/32 KnOTS adapters,
verified (probe~3/4) that they are freely downloadable and load into the harness, and aligned the whole
repository (configs, runners, README) to B/32. This is the single most important setup decision: it makes
every number in the paper directly comparable to the literature.

\subsection{Phase 4, the M0 environment saga (read this before replicating)}
\label{app:m0saga}
Establishing baselines required resolving a genuinely awkward \texttt{torch}/CUDA/\texttt{transformers}
compatibility knot on the free GPU. We document the full sequence of kernel versions because a replicator
will hit the same wall (Table~\ref{tab:m0saga}). The endpoint is: \textbf{use
\texttt{torch==2.6.0+cu124}}, load adapters with \texttt{merge\_and\_unload=true} for the baseline path,
and build the task pool directly from the \texttt{\_template}.

\begin{table}[h]
\centering
\caption{The M0 kernel-version saga. Each failure was diagnosed from the kernel log and fixed in the next
version; only the final two consumed meaningful GPU time (the earlier ones failed fast at load).}
\label{tab:m0saga}
\small
\begin{tabular}{p{0.7cm} p{6.2cm} p{7.4cm}}
\toprule
ver & symptom & cause / fix \\
\midrule
v1 & taskpool was a raw \texttt{DictConfig} (\texttt{Missing key evaluate}); TA/TIES \texttt{\_\_init\_\_}
missing args & Hydra \texttt{defaults:} not resolved $\Rightarrow$ build pool from \texttt{\_template};
supply method args (probe~5) \\
v2 & \texttt{CUDA error: no kernel image available} on P100 & default image ships
\texttt{torch 2.10+cu128} whose binaries drop Pascal \texttt{sm\_60} \\
v3 & same CUDA error after pinning to ``preinstalled'' & the preinstalled build \emph{is} the broken
\texttt{2.10+cu128} \\
v4 & \texttt{torch 2.5.1+cu121} runs on P100, but \texttt{transformers} refuses \texttt{torch.load} of
\texttt{.bin} & CVE-2025-32434 guard requires \texttt{torch>=2.6} for \texttt{.bin} \\
v5 & \texttt{torch 2.6.0+cu124}: \texttt{simple\_average} works; TA/TIES crash on state-dict key/size
mismatch & adapters loaded \texttt{merge\_and\_unload=false} (PEFT-wrapped) can't be subtracted from the
plain pretrained \\
v6 & all three baselines run & load with \texttt{merge\_and\_unload=true} (matches FusionBench's bundled
TA8 config = how published numbers were computed) \\
\bottomrule
\end{tabular}
\end{table}

We added a hard ``GPU sanity gate'' (a tiny embedding\,+\,matmul\,+\,conv on CUDA) at the top of every
GPU kernel so that a mis-matched \texttt{torch} aborts in seconds rather than after the multi-minute
install and data download.

\subsection{Phase 5, baseline tuning and harness validation}
With v6 running, we tuned the baselines. FusionBench's Task Arithmetic \emph{sums} all eight task vectors
then scales, so the optimum is at small $\lambda$: sweeping $\lambda\in\{0.1,0.2,0.3,0.4\}$ gave
$\{0.639,0.566,0.405,0.239\}$, and $\lambda{=}0.1$ reproduces the published Task-Arithmetic-Full number
($0.638$) within $0.1$ point, our harness-validation checkpoint. TIES required the disjoint-\texttt{mean}
combiner (not \texttt{sum}); \texttt{mean} at $\lambda{=}0.3$ gave $0.634$. Specialist accuracies
(Table~\ref{tab:specialists}) reproduce the released fine-tuned numbers. Only after this validation did we
trust subsequent method numbers.

\subsection{Phase 6, M1: optimal-transport averaging (the negative result)}
We then ran our method's first form, OT \emph{averaging} (M-align and M-bary), loading adapters
\emph{unmerged} so the parser reads the factors. Both landed at the floor ($0.610$, $0.622$). A targeted
rank sweep ($R\in\{16,32,64,128\}$) moved the number by $<0.5$ point and larger scaling hurt
(Table~\ref{tab:avgfull}). This is the negative result of Section~\ref{sec:results-averaging}; the
diagnosis (Proposition~\ref{prop:avg}) is that averaging cannot resolve sign interference, so improving
the transport cannot cross the ceiling.

\subsection{Phase 7, M2: the OT-TIES breakthrough, then tuning and peak}
The diagnosis pointed to the fix: keep TIES (which \emph{does} resolve interference) and use OT only to
choose the subspace. We ran a controlled comparison, identical TIES combiner, only the subspace $U$
varies between the SVD of stacked factors (\svdties{}, KnOTS-style) and the OT barycenter (\otties{}).
\svdties{} reached $0.687$ (KnOTS territory, confirming the implementation), and \otties{} reached
$0.696$ at $\lambda{=}0.5$, beating the control. A $\lambda$ sweep showed the OT subspace wins at every
scaling ($+0.7$ to $+1.9$ points); a two-sided reconstruction was worse for both bases and was dropped; a
peak grid over $(\lambda,\kappa)$ placed the optimum at $(0.7,0.2)$ with \otties{}$=0.708$. These are
Tables~\ref{tab:otties}--\ref{tab:peak} and Figure~\ref{fig:otvssvd}.

\subsection{Phase 8, M3: heterogeneous rank, and the code-transport saga}
\label{app:m3saga}
For the heterogeneous-rank carve-out we built a mixed-rank zoo by SVD-truncating the eight rank-16
adapters to ranks $\{4,8,16\}$ (free, no training; Section~\ref{sec:results-hetero}). Running M3 required
getting our \emph{own} package onto the Kaggle worker, which produced its own instructive saga
(Table~\ref{tab:m3saga}). The robust solution was: push the repository to a \emph{public} GitHub and
\texttt{pip install git+https://\dots} with a retry guard; an embedded base64 tarball is \emph{not}
recommended (a $\sim\!20$\,KB blob corrupted in transit, failing \texttt{zlib} decompression). With the
package installed, \otties{} merged the mixed-rank zoo at $0.698$ ($-1.0$ point vs.\ homogeneous), the GW
operator ran at $0.623$, and both merges were pure CPU ($264$ to $359$\,s).

\begin{table}[h]
\centering
\caption{The M3 code-transport saga (getting our own package onto the free worker).}
\label{tab:m3saga}
\small
\begin{tabular}{p{0.7cm} p{6.2cm} p{7.4cm}}
\toprule
ver & symptom & cause / fix \\
\midrule
v1 & \texttt{git clone} of our repo exited 128 $\Rightarrow$ \texttt{ModuleNotFoundError} & repo had
reverted to \emph{private}; anonymous clone fails \\
v2 & same, after a clone-retry guard & still private; retry cannot fix an auth failure \\
v3 & embedded base64 tarball: \texttt{zlib error: invalid code lengths} & a $\sim\!20$\,KB blob inlined in
the notebook corrupts in transit; do not embed \\
v4 & success & set repo \emph{public} (\texttt{gh repo edit --visibility public}); \texttt{pip install
git+\dots} with $5\times$ retry \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Make the code reachable}: push the repo to a public Git host; on the worker,
\texttt{pip install git+<url>} (with a few retries to absorb transient clone failures).
...
baseline path, \texttt{false} (factored) for our OT path.
...
Running the recipe should reproduce: specialist accuracies (Table~\ref{tab:specialists}); the naive floor
$\approx0.64$ with Task Arithmetic at $\lambda{=}0.1$ matching $0.638$; the OT-averaging plateau at
$\approx0.62$; \svdties{}$\approx0.687$ and \otties{}$\approx0.708$ with the OT-over-SVD gap positive at
every $\lambda$; and the mixed-rank \otties{} at $\approx0.698$. Small ($\pm0.005$) deviations are
expected from dataloader and BLAS nondeterminism; the \emph{relative} orderings (OT $>$ SVD $>$ floor;
...
The control \svdties{} is Algorithm~\ref{alg:otties} with line~2 to 3 replaced by
