...
task updates, an algebraic, geometry-agnostic choice. In this project we ask whether
...
Every merge in this project is a sequence of small ($r\times r$ to $k\times k$, $r\le 16$, $k\le 128$)
...
Table~\ref{tab:methodcompare} places the main methods on the axes that matter for this project.
...
($264$ to $359$\,s). This is the capability the rank-fixed baselines cannot offer at all.
...
heterogeneous-rank adapters. For all problems in this project the supports are tiny (rank $r\le16$, or
...
\label{lem:identity}
For $T=1$, every merge operator in this project (M-align, M-bary, \otties{}, GW) returns exactly the input
update $\dW_1$.
\end{lemma}
\begin{proof}
